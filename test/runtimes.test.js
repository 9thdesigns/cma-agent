...
test("Cursor derives the last prompt the same way Claude Code does", () => {
  // Cursor speaks Claude Code's stream-json, so the same shape carries the
  // same meaning — including that a subagent's window is not this one's.
  const out = cursor.collapseEvents([
    { type: "assistant", message: { content: [{ type: "text", text: "Reading…" }],
                                    usage: { input_tokens: 1_000, cache_read_input_tokens: 40_000 } } },
    { type: "assistant", parent_tool_use_id: "toolu_1",
      message: { content: [], usage: { input_tokens: 800_000 } } },
    { type: "assistant", message: { content: [{ type: "text", text: "Done." }],
                                    usage: { input_tokens: 500, cache_read_input_tokens: 61_000 } } },
    { type: "result", subtype: "success", result: "Done.", is_error: false,
      usage: { input_tokens: 2_000, cache_read_input_tokens: 900_000, output_tokens: 300 } }
  ])
...
test("a Cursor build that reports no per-turn usage reports no context reading", () => {
  const out = cursor.collapseEvents([
    { type: "assistant", message: { content: [{ type: "text", text: "Done." }] } },
    { type: "result", result: "Done.", usage: { input_tokens: 400_000 } }
  ])
...
test("CodeWhale sums its per-turn counts and keeps the last one as the reading", () => {
  // `turn_usage` is documented per turn, so a three-turn run spent all three
  // — and sits at the size of the third. Keeping only the last turn (what
  // this adapter used to do) under-reported the run by a factor of its
  // length.
  const out = codewhale.collapseEvents([
    { type: "turn_usage", input_tokens: 10_000, output_tokens: 400, prompt_cache_hit_tokens: 0 },
    { type: "turn_usage", input_tokens: 12_000, output_tokens: 600, prompt_cache_hit_tokens: 8_000 },
    { type: "turn_usage", input_tokens: 3_000, output_tokens: 900, prompt_cache_hit_tokens: 26_000 },
    { type: "done", status: "ok" }
  ])

  assert.equal(out.usage.input_tokens, 25_000, "spend is every turn added together")
  assert.equal(out.usage.output_tokens, 1_900)
  assert.equal(out.usage.cache_read_input_tokens, 34_000)
  assert.equal(out.usage.context_tokens, 29_000, "occupancy is the last turn alone: 3,000 + 26,000")
})
...
test("Codex reports nothing until a build emits the per-request figure", () => {
  ...
  assert.equal(cumulative.usage.input_tokens, 6_900_000, "spend is reported as given")
  assert.equal(cumulative.usage.context_tokens, 0, "and is emphatically not an occupancy")

  // The field that issue asked for, in the shape it asked for it.
  ...
  assert.equal(withLast.usage.context_tokens, 252_301)
})
...
test("Gemini and Antigravity report aggregates, so they report no reading", () => {
  ...
  const a = antigravity.collapseEvents([
    { type: "result", status: "success", usage: { input_tokens: 800_000, output_tokens: 4_000 } }
  ])
...
test("a Grok end event that repeats the answer wins over the streamed chunks", () => {
  const out = grok.collapseEvents([
    { type: "text", text: "partial" },
    { type: "end", text: "the whole answer", status: "success" }
  ])
...
test("Grok's tool calls produce the same ticker lines", () => {
  ...
  assert.equal(line({ type: "tool_call", name: "read_file", args: { path: "app/models/user.rb" } }),
               "Reading models/user.rb")
  assert.equal(line({ type: "tool_call", name: "run_terminal_cmd", args: { command: "cd /r && git status" } }),
               "Running git status")
  assert.equal(line({ type: "tool_call", name: "grep", args: {} }), "Searching the code")
  assert.equal(line({ type: "thought" }), "Thinking")
  assert.equal(line({ type: "text", text: "the answer" }), null, "the answer is not a step")
})

test("a Grok write reports the path it wrote", () => {
  assert.equal(grok.writtenPathFrom({ type: "tool_call", name: "edit_file", args: { path: "docs/plan.md" } }),
               "docs/plan.md")
  assert.equal(grok.writtenPathFrom({ type: "tool_call", name: "read_file", args: { path: "docs/plan.md" } }),
               null)
})
