...
test("the ticker reads the ACP kind when the tool name is not one xAI documents", () => {
  ...
  // xAI documents the shell as run_terminal_cmd, not bash.
  assert.equal(line({ type: "tool_call", toolName: "run_terminal_cmd", kind: "execute", title: "npm test", rawInput: {} }),
               "Running npm test")
  assert.equal(line({ type: "tool_call", toolName: "mcp__cma_github__pr_open", rawInput: {} }),
               "Cma github pr open")
...
test("a nested tool call describes what it is doing, not 'Working'", () => {
  ...
  // Reasoning is the thinking log, not the ticker. A thought event used to
  // print the bare word "Thinking" between every real action; Claude's log
  // is the tool steps, and this ticker is the same surface.
  assert.equal(line({ type: "agent_thought_chunk", content: { type: "text", text: "hmm" } }), null)
  assert.equal(grok.partialThoughtFrom({ type: "agent_thought_chunk", content: { type: "text", text: "hmm" } }), "hmm")
...
// ---------------------------------------------------------------------------
// Shapes 0.22.0 still missed. A real Grok turn after that release still
// showed "Working" and "finished without producing an answer" because:
//
//   * serde adjacent tagging wraps the inner `data` string (`Text { data }`)
//     as `{"type":"text","data":{"data":"hello"}}` — ContentBlock `{type,text}`
//     was handled, this was not, so the answer stayed empty;
//   * a tool call nested two objects down (`data.toolCall.toolName`) still
//     had no name where fieldOf looked, so the ticker said "Working";
//   * a build that stopped projecting emitted ACP `session/update` with the
//     type on `params.update.sessionUpdate`.
// ---------------------------------------------------------------------------

test("adjacent-tagged text still yields the answer", () => {
  const out = grok.collapseEvents([
    { type: "text", data: { data: "Here's " } },
    { type: "text", data: { data: "a summary" } },
    { type: "end", data: { stopReason: "end_turn", sessionId: SESSION, usage: END_SPEND.usage } }
  ])
  assert.equal(out.content, "Here's a summary")
  assert.ok(out.sawResult)
...
test("a twice-nested tool call names the file, not 'Working'", () => {
  const line = (event) => grok.describeEvent(event)
  assert.equal(line({
    type: "tool_call",
    data: { toolCall: { toolName: "read_file", kind: "read", rawInput: { path: "src/main.rs" } } }
  }), "Reading src/main.rs")
  assert.equal(grok.writtenPathFrom({
    type: "tool_call",
    data: { toolCall: { toolName: "search_replace", kind: "edit", rawInput: { path: "lib/a.rb" } } }
  }), "lib/a.rb")
})

test("an ACP session/update still yields the answer and the tool name", () => {
  const stream = [
    { jsonrpc: "2.0", method: "session/update",
      params: { sessionId: SESSION, update: { sessionUpdate: "agent_thought_chunk", content: { type: "text", text: "looking around" } } } },
    { jsonrpc: "2.0", method: "session/update",
      params: { sessionId: SESSION, update: { sessionUpdate: "tool_call", toolName: "read_file", kind: "read", rawInput: { path: "app.rb" } } } },
    { jsonrpc: "2.0", method: "session/update",
      params: { sessionId: SESSION, update: { sessionUpdate: "agent_message_chunk", content: { type: "text", text: "Done." } } } },
    { type: "end", stopReason: "end_turn", sessionId: SESSION, requestId: "r", ...END_SPEND }
  ]

  assert.equal(grok.describeEvent(stream[0]), null, "thoughts are the log, not the ticker")
  assert.equal(grok.partialThoughtFrom(stream[0]), "looking around")
  assert.equal(grok.describeEvent(stream[1]), "Reading app.rb")
  assert.equal(grok.collapseEvents(stream).content, "Done.")
})

test("thought text is never collected as the answer", () => {
  const out = grok.collapseEvents([
    { type: "thought", data: { data: "the user probably wants X" } },
    { type: "end", stopReason: "end_turn", sessionId: SESSION, requestId: "r", ...END_SPEND }
  ])
  assert.equal(out.content, "")
  assert.equal(grok.partialThoughtFrom({ type: "thought", data: { data: "the user probably wants X" } }),
