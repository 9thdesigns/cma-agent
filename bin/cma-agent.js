...
import { loginProfile, probeProfile, runtimeVersion } from "../src/engine.js"
...
import {
  addProfile, defaultProfileSlug, describeAccount, duplicateAccounts, listProfiles, loginSlug,
  removeProfile, scanProfiles, runtimeCatalogue, runtimeModels, runtimeVersions, setDefaultProfile,
...
import { applyUpdates, checkForUpdates, isHosted, RESTART_EXIT_CODE, updateWanted } from "../src/hosted.js"
...
Logins
  ...
  runtimes:login [--runtime cursor] [--profile work] [--no-start] [--device-auth]
                                Sign a login in again after it expires, then
                                  start the companion. With no --profile this
                                  signs in the login unnamed work runs on —
                                  the one \`cma-agent use\` chose. --device-auth
                                  signs in without a browser where the runtime
                                  offers that (Grok Build): it prints a URL and
                                  a code to enter on another device.
...
Cloud machines
  hosted:update                 On a machine Configure My AI runs (CMA_HOSTED=1):
                                  install whatever the server says is behind.
                                  \`start\` does this by itself daily.
...
async function cmdVerify() {
  try {
    const result = await api.verify({
      ...
      runtimeModels: await runtimeCatalogue()
    })
...
async function cmdRuntimeAdd(args, fallbackRuntime = null) {
  ...
  // The vendor's sign-in for Claude Code is its whole console, and nothing in
  // it says how to hand the terminal back.
  console.log(`When it says you are signed in, leave that window (\`exit\`, or Ctrl-D).\n`)

  await loginProfile(runtime, profile.slug, { deviceAuth: args["device-auth"] === true })
...
async function cmdRuntimeLogin(args, fallbackRuntime = null) {
  ...
  // Where the sign-in actually lands, which with no --profile is where work
  // that names no login runs — NOT the ambient login. See loginSlug: signing
  // in the ambient one while every run uses a profile directory is what turned
  // an expired session into a loop of "sign in again" that never fixed it.
  const slug = loginSlug(runtime, args.profile)
  const known = listProfiles(runtime)

  if (slug && !known.some((p) => p.slug === slug)) {
    console.error(`✗ No ${runtime.name} login called "${slug}" on this machine.`)
    console.error(`  Try: ${["default", ...known.map((p) => p.slug)].join(", ")}`)
    console.error("  Or see them all with: cma-agent accounts")
    return 1
  }

  const label = slug ? (known.find((p) => p.slug === slug)?.label || slug) : DEFAULT_PROFILE.label
  const account = describeAccount(runtime, slug)
  // Whether this command ends by taking work, rather than by telling someone
  // to run one more thing. See the bottom of this function.
  const autoStart = !args["no-start"] && stdin.isTTY === true

  // Say which login is about to be signed in, before handing the terminal to
  // the vendor's own flow. The vendor prints its own "Welcome back" against
  // whatever account it resolves, and that greeting looks identical whichever
  // directory the credential lands in — so this line is the only thing on
  // screen that distinguishes the two.
  console.log(`Signing in ${runtime.name} · ${label} [${slug || "default"}]${account ? ` — ${account}` : ""}`)
  if (slug) console.log(`  Credential goes in ${profileDir(runtime, slug)}`)
  if (slug && !args.profile) {
    console.log("  This is the login that work naming no login runs on (`cma-agent use`).")
  }

  // We hand the terminal to the vendor's own sign-in, which for Claude Code is
  // its full console — and nothing in it says how to get back, so people sat
  // in a chat session wondering whether the sign-in had taken. Say it before
  // handing over, and say what happens after, because the next thing that
  // happens is this process taking over the window again.
  console.log(`  Sign in in the ${runtime.name} window that opens, then leave it (\`exit\`, or Ctrl-D).`)
  if (autoStart) console.log("  The companion starts here by itself once you are back.\n")

  // --device-auth is passed through to the runtimes that have a no-browser
  // flow; the rest ignore it. See loginProfile.
  await loginProfile(runtime, slug, { deviceAuth: args["device-auth"] === true })

  // Check rather than congratulate. `loginProfile` resolves on the CLI's exit
  // code, and a Claude Code session that exits cleanly having refreshed
  // nothing exits 0 all the same — which is how "✓ Signed in." got printed
  // four times over a login that was still expired.
  const probe = await probeProfile(runtime, slug)
  const failure = await reportProfiles()

  if (probe.status !== "ready") {
    console.error(`\n✗ "${label}" still isn't signed in — ${String(probe.status).replace(/_/g, " ")}.`)
    if (probe.detail) console.error(`  ${probe.detail}`)
    console.error(`  Try again with: cma-agent runtimes:login --runtime ${runtime.id} --profile ${slug || "default"}`)
    return 1
  }

  const signedIn = describeAccount(runtime, slug)
  console.log(`✓ Signed in: ${label} [${slug || "default"}]${signedIn ? ` — ${signedIn}` : ""}`)

  if (failure) {
    console.error(`! Signed in here, but Configure My AI wasn't told: ${failure}`)
    console.error("  Fix that, then run: cma-agent runtimes:scan")
    return 1
  }

  // Signing a login in is never the goal — running work on it is, and that
  // needs a companion. Somebody who has just fixed an expired login wants the
  // machine taking work again, not a second command to remember while the
  // terminal is still theirs.
  //
  // Not unconditional: `--no-start` is for a setup script, and a
  // non-interactive shell must not be handed a process that never exits. Both
  ...
  console.log("\nStarting the companion — leave this window open (Ctrl-C to stop).\n")
  return start()
}
...
async function reportProfiles(profiles) {
  try {
    ...
    await api.syncProfiles(profiles || (await scanProfiles()), await runtimeCatalogue())
    return null
  } catch (error) {
...
async function cmdUse(args) {
  ...
  if (deviceToken()) {
    try {
      await api.syncProfiles(await scanProfiles(), await runtimeCatalogue())
...
async function main() {
  ...
  switch (command) {
    ...
    case "hosted:update": {
      if (!isHosted()) {
        console.error("This machine is not one Configure My AI runs (CMA_HOSTED is not set). On your own computer, update with the commands on the Devices page.")
        return 1
      }
      ...
      return (await applyUpdates(plan, console.log)) ? RESTART_EXIT_CODE : 1
    }
