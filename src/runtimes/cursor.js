...
import {
  classifyFailure, emptyUsage, envForGithub, envForWeb, FORBIDDEN_GIT, GIT_VERBS, loginCommand,
...
function collapseEvents(events) {
  ...
  // which is what `result.usage` carries. Cursor speaks Claude Code's
  // stream-json, so the derivation is the same one and lives beside the same
  // reasoning: see lastTurnOccupancy in shared.js.
  //
  // Cursor's own builds have not been confirmed to populate message.usage on
  // assistant events the way Claude Code does. That is why this is written as
  // "use it if it is there": on a build that reports it the meter is exact,
  // and on one that doesn't the value stays 0 and the composer draws no meter
  // rather than drawing the run total.
  let contextTokens = 0
  ...
  // last, so assigning this inside it would hand the server the run total
