...
function tokensFrom(stats) {
  ...
  for (const candidate of candidates) {
    ...
    return {
      ...
      // `cachedContentTokenCount` is the API's name; `cached` is what the
      // CLI's own stats.tokens shape calls the same number. Reading only the
      // API spelling dropped the cache figure on exactly the builds whose
      // prompt/candidates spellings the two lines above accept — so a Gemini
      // run's cache traffic reached the server as zero and the session meter
      // showed no measured split.
      cache_read_input_tokens: Number(
        candidate.cachedContentTokenCount ?? candidate.cached_content_token_count ??
        candidate.cached_tokens ?? candidate.cached ?? 0
      )
    }
  }

  // The current CLI's JSON nests per model — stats.models[<model>].tokens
  // carrying {prompt, candidates, cached, …} — a shape none of the flat
  // candidates above can match, because stats.models' own keys are model
  // NAMES. A run can touch more than one model (flash for classification,
  // pro for the answer), so the totals are summed across all of them.
  if (stats.models && typeof stats.models === "object") {
    ...
    for (const entry of Object.values(stats.models)) {
      ...
      if (tokens.prompt === undefined && tokens.candidates === undefined) continue
      seen = true
    ...
    if (seen) {
      return { input_tokens: prompt, output_tokens: output, cache_read_input_tokens: cached }
    }
  }
...
function collapseEvents(events) {
  ...
  for (const event of events) {
    ...
    if (event?.type === "result") {
      ...
      // `stats` is the SESSION's accounting — promptTokenCount there is every
      // API call the run made added together, so on a multi-step run it is a
      // multiple of any prompt that existed and must not be read as how full
      // the window is. Gemini CLI's own /stats view is built from the same
      // aggregate. If a build starts reporting the last request separately,
      // this picks it up under any of the usual spellings; until then the
      // value stays 0 and the composer draws no meter rather than a wrong one.
