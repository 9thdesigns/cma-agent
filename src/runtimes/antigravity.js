import {
  classifyFailure, emptyUsage, lastTurnOccupancy, locateBin, locationAdvice, normalizeUsage,
...
function collapseEvents(events) {
  ...
  for (const event of events) {
    ...
    if (event?.type === "result") {
      ...
      // The result event reports what the run spent in total, over however
      // many API calls `agy` made — not the size of the last prompt, which is
      // what a context meter needs. Probed rather than derived, because this
      // adapter has never seen a per-request figure in the wild; 0 until a
      // build emits one, and 0 draws no meter.
      out.usage.context_tokens = lastTurnOccupancy(event)
