...
//
// So a field is read from the event, and then — only if it is not there — from
// whichever single member holds a nested payload. The flat shape is unchanged
// by this (the first lookup finds everything), a nested one now reads, and
// neither can invent a value the line did not carry.
const NESTED_KEYS = ["data", "toolCall", "tool_call", "update", "payload"]
...
function textOf(event) {
  if (!TEXT_TYPES.has(event?.type)) return null

  for (const key of TEXT_KEYS) {
    const chunk = textFromContent(event[key])
    if (chunk) return chunk
  }
  // Last resort, and only for a nested payload: a build that wraps the block
  // under a member this file does not name still hands over its text.
  const chunk = textFromContent(nestedOf(event))
  return chunk || null
}
...
function describeEvent(event) {
  ...
  switch (event.type) {
    // The projected spellings, then the raw ACP `sessionUpdate` names a build
    // that stops projecting would emit instead.
    case "thought":
  ...
  const pretty = (toolNameOf(event) || toolTitleOf(event)).replace(/_/g, " ").trim()
