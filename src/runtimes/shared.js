...
export const BUILTIN_WEB_TOOLS = {
  ...
  antigravity: [],
  codex: [],
  // grok — `web_search` is named in xAI's own `--disallowed-tools` example;
  // `web_fetch` is inferred from the documented `WebFetch(...)` permission
  // prefix and has not been read off a build. An unknown name is inert to a
  // deny list, so the guess costs nothing, and the adapter ALSO sets
  // `disable_web_search = true` in config.toml — a documented key, which is
  // the part actually doing the work here.
  grok: ["web_search", "web_fetch"],
  codewhale: [],
  ollama: []
}
...
// Features Configure My AI OWNS, per runtime: the same needs a bot's own tools
// cover — scheduling, sub-agents, workflows, worktrees, plan mode, memory,
// question prompts — that a CLI also offers as built-ins. A bot turn that used
// one did the work somewhere the operator cannot see: a job "scheduled" in a
// CLI's session-only cron died with the session. The prompt says not to
// (Autopilot::CapabilityManifest.tool_authority, the same band on every
// provider); this takes them off the surface so the model cannot. Same
// confidence notes as the web list:
//   claude_code — names read off the build's own tool list (2.1.x). An
//     unknown name is inert to --disallowedTools, so an older build without
//     one of these loses nothing.
//   gemini_cli  — save_memory is Gemini's documented memory tool, and memory
//     is the bot's (memory_update). Not machine-checked.
//   the rest    — empty, for the reasons given above.
export const BUILTIN_PLATFORM_TOOLS = {
  claude_code: [
    "CronCreate", "CronDelete", "CronList", "ScheduleWakeup",
    "Agent", "Workflow", "EnterWorktree", "ExitWorktree",
    "EnterPlanMode", "ExitPlanMode", "AskUserQuestion",
    "Monitor", "SendMessage", "PushNotification", "ListAgents", "Skill"
  ],
  gemini_cli: ["save_memory"],
  cursor: [],
  antigravity: [],
  codex: [],
  // grok — `Agent` is xAI's own documented example of blocking sub-agent
  // spawning (`--disallowed-tools "Agent"`), and the adapter turns the feature
  // off in config.toml as well. `ask_user_question` is named in grok's config
  // reference as a toolset but its tool name has not been verified, so it is
  // left out rather than guessed: an inert entry reads as coverage.
  grok: ["Agent"],
  codewhale: [],
  ollama: []
}

export function builtinPlatformTools(runtimeId) {
  return BUILTIN_PLATFORM_TOOLS[runtimeId] || []
}
