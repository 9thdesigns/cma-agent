...
function collapseEvents(events) {
  ...
  for (const event of events) {
    ...
    // `turn_usage` is documented as PER TURN, which makes this the one
    // adapter that gets both numbers straight from the stream — and the one
    // place the two must not be conflated:
    //
    //   totals    every turn added together. This used to keep only the last
    //             turn's counts, which under-reported a ten-step run by
    //             roughly a factor of ten on the Usage page.
    //   occupancy the LAST turn on its own — how full the window was on the
    //             prompt that just went out, which is a level and not a sum.
    //
    // If a build ever emits cumulative counts under this name, the totals
    // would double-count; the event name and CodeWhale's own documentation
    // both say per-turn, and that is what the adapter contracts on.
    if (event?.type === "turn_usage") {
      const turn = normalizeUsage(event)
      out.usage = {
        input_tokens: out.usage.input_tokens + turn.input_tokens,
        output_tokens: out.usage.output_tokens + turn.output_tokens,
        cache_creation_input_tokens:
          out.usage.cache_creation_input_tokens + turn.cache_creation_input_tokens,
        cache_read_input_tokens:
          out.usage.cache_read_input_tokens + turn.cache_read_input_tokens,
        context_tokens: occupancyOf(turn) || out.usage.context_tokens
      }
    }
