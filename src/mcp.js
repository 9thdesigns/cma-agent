...
function toolsForSession() {
  ...
  const description =
    ...
    `Optional; defaults to ${primary}. Pushing operations (local_push, pr_open) commit and ` +
    `push the named repository's OWN checkout on this machine. A change spanning ` +
    `repositories needs one call per repository, so pass this explicitly rather than ` +
