...
import { isHosted, subscriptionFromProbe } from "./hosted_status.js"
...
export async function scanProfiles() {
  ...
  for (const runtime of installedRuntimes()) {
    for (const profile of [DEFAULT_PROFILE, ...listProfiles(runtime)]) {
      ...
      // On a cloud machine, what the probe said about the SUBSCRIPTION —
      // a rate limit and an expired login both fail a probe, and the web
      // app has to send the person to opposite places. A laptop's owner
      // reads the vendor's words in their own terminal and gets nothing
      // extra here.
      const subscription = isHosted() ? subscriptionFromProbe(probe) : null
      report.push({
        runtime: runtime.id,
...
// The same map, plus what the machine knows ABOUT those models, for the
// server. A runtime that can describe its models (`listModelMeta`: what each
// one can do, how long a prompt it takes, whether it is really on this disk
// or a cloud stub) reports that under one reserved key, `_meta`, keyed by
// runtime and then by model name, so names and descriptions always travel
// together and a scan can never describe a model it did not list.
//
// The key is present whenever a runtime could answer at all — an empty
// description is an answer ("this Ollama describes nothing"), and the
// server treats the key's absence, not its emptiness, as silence. A server
// too old to know the key drops it as an unknown runtime.
export const MODEL_META_KEY = "_meta"

export async function runtimeCatalogue() {
  const models = await runtimeModels()
  const meta = {}
  let described = false

  for (const runtime of installedRuntimes()) {
    if (!models[runtime.id] || typeof runtime.listModelMeta !== "function") continue
    described = true
    try {
      meta[runtime.id] = await runtime.listModelMeta()
    } catch (_error) {
      meta[runtime.id] = {}
    }
  }

  if (described) models[MODEL_META_KEY] = meta
  return models
}
