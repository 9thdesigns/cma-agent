...
import {
  DEFAULT_PROFILE, defaultProfileSlug, describeAccount, listProfiles, resolveSlug, runtimeCatalogue,
  runtimeVersions, scanProfiles
} from "./profiles.js"
import { getRuntime, installedRuntimes, isInstalled, RUNTIMES } from "./runtimes/index.js"
import { loginCommand } from "./runtimes/shared.js"
import { resolveRepo, runCommand, COMMANDS } from "./repos.js"
import * as hosted from "./hosted.js"
import { classifySubscription, isHosted, LOGIN_KINDS } from "./hosted_status.js"
...
export async function start({ log = console.log } = {}) {
  ...
  let models = await runtimeCatalogue()
  ...
  if (isHosted()) {
    log(`Hosted by Configure My AI — sleeps after ${Math.round(hosted.IDLE_MS / 60000)} idle minutes, checks for updates daily.`)
  }
  ...
  let running = true
  // What the process ends with. A hosted machine ends with 0 to stop the
  // machine (idle) or RESTART_EXIT_CODE to be started again (updated).
  let exitCode = 0
  const stop = () => {
    running = false
  ...
  // How busy this machine is, for the hosted supervisor: how many runs are
  // in flight across both lanes, and when the last one started or ended.
  const activity = { inflight: 0, lastJobAt: Date.now() }
  let supervisor = null
  if (isHosted()) {
    supervisor = hosted.supervise({
      activity, log,
      exit: (code) => {
        exitCode = code
        running = false
  ...
  const beat = async () => {
    try {
      ...
      // A cloud machine hears about a newer companion here, within a
      // heartbeat of the release, instead of at its next daily check.
      if (data?.companion?.update) supervisor?.nudge()
    } catch (error) {
      if (error.status === 409) {
  ...
  // Returns what the scan found, so startup can SAY it. A report that only
  // ever went to the server is why a signed-out login could sit here looking
  // healthy: the terminal listed its account and never mentioned that every
  // run sent to it would fail.
  const rescan = async () => {
    let profiles = null
    try {
  ...
  await beat()
  warnSignedOut(await rescan(), log)
  ...
  try {
    await Promise.all([
      ...
      poll({ lane: "completions", limit: () => limits.jobs, log, isRunning: () => running, onUnpaired: unpaired, activity }),
      poll({ lane: "commands", limit: () => MAX_COMMAND_JOBS, log, isRunning: () => running, onUnpaired: unpaired, activity })
    ])
  } finally {
...
async function poll({ lane, limit, log, isRunning, onUnpaired, activity = null }) {
  const inflight = new Set()
  let backoff = BACKOFF_START_MS
  // Said once per stretch, not once per pass. A lane with every slot taken
  // stops claiming, and until it said so that was indistinguishable from a
  // lane with nothing to do — which is how a machine could sit here looking
  // idle and healthy while every coding turn sent to it expired in the queue.
  // The two lanes have separate slots, so one wedged lane leaves the other
  // visibly answering, and a terminal that says nothing cannot tell you that.
  let saturated = false
  ...
  while (isRunning()) {
    if (inflight.size >= limit()) {
      if (!saturated) {
        saturated = true
        log(`· Running ${inflight.size} at once — waiting for one to finish before taking more ${lane}.`)
      }
    ...
    if (saturated) {
      saturated = false
      log(`· A slot freed up — taking ${lane} again.`)
    }

    let claimed = null
    try {
      claimed = await api.claimJob({ wait: 25, lane })
    ...
    if (!job) {
      // An empty poll may carry a pause: the server is crowded and asked us
      // to breathe before polling again. Sleeping here is what turns "every
      // companion holds a server thread at all times" into a duty cycle the
      // server sized itself — dispatch merely waits for our next poll, which
      // the cap keeps close. Zero (the usual case) polls again immediately,
      // exactly as before.
      if (claimed.pauseMs > 0) await sleep(Math.min(claimed.pauseMs, 60000))
      continue
    }
...
// Logins that are signed out, and the command that fixes each one.
//
// The scan already probes every login at startup — it just had nowhere to say
// so. A machine whose "9th" login had expired printed `· Claude Code · 9th
// [9th] — account: you@acme.com  ← work with no login named lands here`, which
// reads as healthy, and the first failure arrived minutes later in a browser.
//
// Deduplicated by the directory each row actually probes: when `cma-agent use`
// points the ambient row at a named login, both rows describe the same
// credential and warning twice about one expired session is noise. The named
// row wins because its slug is what the fix command needs.
//
// Exported for the tests: the dedup and the profile in the command are the
// whole point, and neither should be reachable only by starting a companion.
export function signedOutLogins(profiles) {
  const rows = [...(profiles || [])].sort((a, b) => Number(a.slug === "default") - Number(b.slug === "default"))
  const seen = new Set()
  const out = []

  for (const profile of rows) {
    if (profile.status !== "logged_out") continue

    // "default" is the wire's word for the ambient row, and the ambient row
    // probes wherever `cma-agent use` pointed it.
    const slug = profile.slug === "default" ? defaultProfileSlug(profile.runtime) : profile.slug
    const key = `${profile.runtime}:${slug}`
    if (seen.has(key)) continue
    seen.add(key)

    // Not every runtime's sign-in is one we can drive: Gemini, Antigravity and
    // CodeWhale are signed in by running their own CLI once, and they say so
    // on their adapter. Printing `cma-agent runtimes:login` for those would be
    // a command that refuses — the exact failure mode this whole change is
    // about, one runtime over.
    const runtime = getRuntime(profile.runtime)
    const drivable = runtime?.loginArgs?.()?.length > 0
    ...
    out.push({
      ...
      command: drivable || !runtime ? loginCommand(profile.runtime, slug) : runtime.loginHint
    })
  }
...
function warnSignedOut(profiles, log) {
  for (const login of signedOutLogins(profiles)) {
    const name = getRuntime(login.runtime)?.name || login.runtime
    log(`! The ${name} login "${login.label}" is signed out — work sent to it will fail.`)
    if (login.detail) log(`    ${login.detail}`)
    log(`    Sign it in with: ${login.command}`)
  }
}
...
async function handleJob(job, log) {
  ...
  // A cloud machine's sign-in relay: start the vendor's no-browser flow,
  // hand it the code the person pasted, or stop it. Refused on a laptop,
  // where the vendor's own sign-in in a real terminal is the right tool.
  const kind = job.kind || "completion"
  if (LOGIN_KINDS.has(kind)) {
    log(`→ ${kind}…`)
    try {
      if (!isHosted()) throw new Error("Sign-in by relay only runs on a cloud machine. Sign in here with `cma-agent runtimes:login`.")
      const data = await hosted.handleLoginJob(job, log)
  ...
  // Set from the progress ack when the SERVER ends the run, so the failure
  // line below reports what actually happened rather than the engine's one
  // guess about why its child was killed.
  let stoppedBy = null
  log(
  ...
  try {
      ...
      { ...job, runtime: runtime.id, workdir, extraWorkdirs, profileSlug: slug },
      {
        ...
        onProgress: (note, { repeat, partial, cancel } = {}) => {
          ...
          api.reportProgress(job.id, note, { repeat, partial })
            .then((data) => {
              const reason = serverStopReason(data)
              if (!reason || !cancel) return
              // Remembered so the line that ENDS the run says the same thing
              // as the line that stopped it. The engine can only report that
              // it was killed, which it renders as "cancelled from the web
              // app" — the one sentence we now know may be untrue.
              stoppedBy = reason
              log(`  · ${reason} — stopping ${runtime.name}.`)
              cancel()
            })
            .catch(() => {})
        }
    ...
    //
    // `stoppedBy` wins when the server is the one that ended this run: the
    // engine only knows its child was killed and says so in the words of the
    // one cause it used to assume, which is the report that sent people
    // looking for a Stop button nobody had pressed.
    const reported = stoppedBy || error.message
    // On a cloud machine, what this failure says about the plan — a rate
    // limit, an expired sign-in, a billing refusal — rides with it, so the
    // web app can show a status and a button instead of a sentence.
    const subscription = isHosted() && !stoppedBy ? classifySubscription(reported) : null
    try {
      await api.submitResult(job.id, subscription ? { error: reported, subscription } : { error: reported })
    } catch (_reportError) {
      log(`! Couldn't report the failure for job ${job.id}`)
    }
    log(`✗ ${reported}`)
...
// States the SERVER can reach while this machine is still running, and what
// each one means to the person watching this terminal.
//
// Only "claimed" means "keep going". A run the server has already settled is
// one whose answer nobody will read, so continuing spends the user's own
// subscription producing output that is thrown away on arrival — which is what
// happened to every `expired` run, because this loop only ever looked for
// "cancelled".
//
// The fallbacks are for a server too old to send a reason. A newer one composes
// the sentence itself, because the reason is not derivable here: `cancelled` is
// what a person pressing Stop produces AND what the server produces when it
// gives up waiting, and reporting the second as the first told people they had
// cancelled a run they never touched.
const SERVER_STOPPED = {
  cancelled: "Cancelled from the web app",
  expired: "The server gave up on this run",
  failed: "The server recorded this run as failed"
}

// Exported for the tests: the mapping between a server state and what this
// terminal says about it is the whole point of the change, and it should not
// only be reachable by running a job against a live server.
export function serverStopReason(data) {
  const state = data?.state
  // hasOwn, not `in`: every object answers to "constructor" and "toString",
  // and a state we did not write must never read as a reason to kill a run.
  if (!state || !Object.hasOwn(SERVER_STOPPED, state)) return null
  return String(data.reason || "").trim() || SERVER_STOPPED[state]
}
