...
// Carry the working tree onto `branch`, whether or not that branch exists yet
// and whether or not the tree is dirty.
//
// `git checkout` REFUSES to switch when a locally-modified file differs between
// HEAD and the target, and for this caller that is the normal case rather than
// an edge one: the second push of a session lands on a branch that already
// holds the first push's version of the very files the run has just edited
// again. So the old order — checkout, then add, then commit — failed precisely
// when it was doing its job. A real run's log shows it:
//
...
//
// three times in a row, each one a tool call the model paid for and had to work
// around, before a later attempt happened to find the tree agreeable.
//
// So the work is moved deliberately instead of being left for `checkout` to
// refuse: stashed (untracked files included, or a file the run CREATED would be
// left behind), switched with a clean tree, restored on the other side. A
// restore that conflicts is resolved in favour of THE RUN — its edits are the
// intended new state, and the branch's older copy of the same file is exactly
// what they are there to replace.
async function switchTo(cwd, branch, { exists, base }) {
  const target = exists ? ["checkout", branch] : ["checkout", "-b", branch, ...(base ? [base] : [])]

  // A new branch off HEAD keeps the very commit the tree was built on, so no
  // file can differ and there is nothing to carry. Same for a clean tree.
  if (!exists && !base) return git(cwd, target)
  if (!(await statusOf(cwd)).dirty) return git(cwd, target)

  const stashed = await stashWork(cwd, branch)
  const checkout = await git(cwd, target)
  if (!checkout.ok) {
    // Leave the machine as we found it. A failed push must not also be a push
    // that lost the work it was asked to carry.
    if (stashed) await git(cwd, ["stash", "pop"])
    return checkout
  }

  if (stashed) await restoreWork(cwd)
  return checkout
}

async function stashWork(cwd, branch) {
  const out = await git(cwd, ["stash", "push", "--include-untracked",
                              "-m", `cma-agent: carrying work to ${branch}`])
  // "No local changes to save" is a success with nothing stashed, and popping
  // then would take somebody else's entry off the stack.
  return out.ok && !/No local changes/i.test(`${out.stdout}${out.stderr}`)
}

async function restoreWork(cwd) {
  const pop = await git(cwd, ["stash", "pop"])
  if (pop.ok) return

  // Conflicted, which here has one right answer. In a stash pop OURS is the
  // branch and THEIRS is the stash, so `--theirs` is the run's own work.
  ...
  // A conflicted pop does NOT drop its entry. Left on the stack it would be
  // replayed onto the next push, which is how one run's edits reappear inside
  // an unrelated one.
...
export async function push(params) {
  ...
  const checkout = await switchTo(cwd, branch, { exists, base })
  if (!checkout.ok) {
...
// The folder name is a contract, not a convenience: `config-my-ai-workspace-`
// plus this machine's device id, so someone looking at a crowded Desktop knows
// exactly which app made the folder and which pairing it belongs to.
export const DESKTOP_WORKSPACE_PREFIX = "config-my-ai-workspace-"

// Where this machine's Desktop is. CMA_DESKTOP_DIR overrides for people who
// want the workspace somewhere else (and for tests, which must not write to a
// real Desktop). Linux desktops relocate or translate ~/Desktop through
// xdg-user-dirs, so that file is consulted before assuming the English name.
// A machine with no Desktop at all — a headless box — falls back to the home
// directory, which is the next most findable place.
export function desktopDir() {
  const override = process.env.CMA_DESKTOP_DIR
  if (override) return path.resolve(expandHome(override))

  const home = os.homedir()
  if (os.platform() === "linux") {
    try {
      const conf = fs.readFileSync(path.join(home, ".config", "user-dirs.dirs"), "utf8")
      const match = conf.match(/^XDG_DESKTOP_DIR="([^"]+)"/m)
      if (match) {
        const resolved = match[1].replace("$HOME", home)
        if (fs.existsSync(resolved)) return resolved
      }
    } catch {
      // No xdg-user-dirs config is the common case, not an error.
    }
  }
...
// A workspace on the machine's own Desktop, made when the user presses the
// button in the web app. ~/.configure-my-ai/workspaces works, but nobody who
// doesn't already know it exists will ever find it; this is the same idea
// somewhere a person actually looks.
//
// This is the one command that touches a path outside the shared roots and
// WORKSPACES_DIR, so the reasoning is spelled out:
//
//   * The server sends NO path and NO name. Everything is composed here, from
//     the prefix constant, the device id in our own config file, and this
//     machine's idea of where its Desktop is. Params are ignored on purpose.
//   * The directory is one we create (or created on an earlier press). Sharing
//     it exposes nothing that already existed on the machine — the command is
//     `mkdir` plus `repos:add`, done for the user rather than by them, and the
//     button press in the web app is that user asking for exactly this.
//
// Idempotent: a second press finds the folder, re-shares it if needed, and
// reports `created: false`.
async function desktopWorkspace() {
  const deviceId = String(readConfig().device_id || "").trim()
  if (!/^[A-Za-z0-9-]{1,64}$/.test(deviceId)) {
    throw new Error("This machine has no device id on record. Run `cma-agent pair` again, then retry.")
  }

  const parent = desktopDir()
  const dir = path.join(parent, `${DESKTOP_WORKSPACE_PREFIX}${deviceId}`)

  const created = !fs.existsSync(dir)
  fs.mkdirSync(dir, { recursive: true, mode: 0o700 })

  // A real repository, not just a folder: everything downstream — the panel's
  // refresh, the session's working tree, an eventual push — speaks git.
  if (!isRepo(dir)) {
    const init = await git(dir, ["init", "--initial-branch=main"])
    if (!init.ok) await gitOrThrow(dir, ["init"]) // a git too old for --initial-branch
  }
  ...
  if (!fs.existsSync(readme)) {
    fs.writeFileSync(
      readme,
      "# Configure My AI workspace\n\n" +
        "Created by Configure My AI for sessions running on this machine.\n" +
        `Device: ${deviceId}\n\nWork done in those sessions lands here.\n`,
      { mode: 0o600 }
    )
  }

  // A first commit, so the repository has a real branch and an honest history
  // instead of an unborn HEAD the panel would render as "(detached)". Only OUR
  // file is staged — never `add -A`: if the folder somehow pre-exists with the
  // user's files in it, sweeping those into a commit uninvited is exactly the
  // kind of reach this file exists to refuse.
  const hasHead = (await git(dir, ["rev-parse", "--verify", "--quiet", "HEAD"])).ok
  if (!hasHead) {
    await gitOrThrow(dir, ["add", "--", "README.md"])
    const commit = await git(dir, ["commit", "-m", "Initialize Configure My AI workspace"])
    if (!commit.ok) {
      // Most likely a machine with no git identity configured. The scaffold
      // commit gets ours rather than failing; the user's own commits keep
      // whatever identity they set up.
      await gitOrThrow(dir, [
        "-c", "user.name=Configure My AI",
        "-c", "user.email=noreply@configuremyai.com",
        "commit", "-m", "Initialize Configure My AI workspace"
      ])
    }
  }

  // Shared like any repos:add folder, so repos.list finds it and every later
  // git.* command resolves it. addRoot de-duplicates, so re-pressing is free.
  addRoot(dir)

  return { ...(await describe(dir)), created, desktop_dir: parent }
}
...
export const COMMANDS = {
  ...
  "workspace.desktop": desktopWorkspace,
  "git.summary": summary,
  "git.log": log,
  "git.diff": diff,
  "git.push": push,
