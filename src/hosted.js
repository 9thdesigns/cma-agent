...
export function submitLoginCode({ session, code }) {
  ...
  state.child.stdin.write(`${String(code).trim()}\n`)
...
export async function checkForUpdates() {
  try {
    ...
    return await api.hostedUpdates({ agentVersion: VERSION, runtime: runtime?.id || null, runtimeVersion: version })
  } catch {
    return null
  }
}

export function updateWanted(plan) {
  return Boolean(plan && ((plan.agent && plan.agent.update && plan.agent.command) ||
                          (plan.runtime && plan.runtime.update && plan.runtime.command)))
}
