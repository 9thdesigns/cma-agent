...
| Antigravity | `agy` | Free Google account, or Google AI Pro / Ultra | Read files; editing needs a rule you add — see below |
| Grok Build | `grok` | SuperGrok, SuperGrok Plus or X Premium+ | Edit files, drive git, open PRs |
...
**Antigravity (`agy`) is chosen the same way as Gemini, and grants less.**
`--mode=accept-edits` looks like Claude Code's `acceptEdits` and is not: it
skips the interactive diff review, and the `write_file` permission check
still runs. A headless run has nobody to approve it, so agy auto-denies the
write and finishes clean-looking but empty — exit 0, status `SUCCESS`, no
answer, `denied_actions: [write_file]`. The grant that actually works is an
allow rule in your own `~/.gemini/antigravity-cli/settings.json`:
`write_file(<the shared folder>)` under `permissions.allow`. That file is
yours and this companion never writes into it, so until you add the rule an
Antigravity repository turn reads the checkout and reports back without
changing it — and a turn that was stopped that way says which rule is
missing rather than going quiet. `run_command` stays behind an approval no
headless run can grant, so the machine pushes for it. The flag agy offers to
lift everything (`--dangerously-skip-permissions`) is never used — that is
its yolo mode. One Antigravity login per machine (the env var that would
isolate a second one is not verified), signed in by running `agy` once in a
terminal.
...
**Grok Build (`grok -p`) is the one runtime since Claude Code that takes the
allowance as flags.** Codex and CodeWhale can only be bounded by a sandbox and
a system prompt, because neither publishes a per-command permission flag; xAI
does. A repository turn passes `--allow 'Bash(git commit*)'` for each git verb
it needs and `--deny 'Bash(git push --force*)'` for each one that destroys
work, and xAI documents that deny beats every other rule and that a
repository's own `.grok/config.toml` cannot widen what we passed — so the
`--yolo` those turns run with cannot re-open what we closed. Plain chats run
`--sandbox read-only` with no allowance at all. Like Codex, there is no
ambient login: MCP servers come from `config.toml` inside `GROK_HOME`, so every
Grok login lives in a directory we own and is signed in once with
`cma-agent runtimes:login --runtime grok` — add `--device-auth` on a machine
with no browser and it prints a URL and a code to enter on another device.

It spends the SUBSCRIPTION and nothing else, which takes one deliberate step:
xAI's credential order ends at `XAI_API_KEY` from the environment, so a profile
whose session had lapsed would otherwise fall through to per-token billing on a
separate account — one a consumer Grok plan cannot even fund. The adapter
removes that variable from the environment it runs `grok` in, so a run either
uses the login you made or stops and says it needs one. To spend an xAI API key
on purpose, use an xAI provider in the web app instead of a local machine one.

One thing is worth knowing before you pick it: the prompt travels as a
command-line argument (visible in `ps` on your own machine, and bounded by
ARG_MAX on a very long conversation). The token counts are read off the
event the CLI's own source defines, including the cached share — a Grok run
reports what it spent like every other runtime here.
...
> **Cursor, Codex and Grok Build have no ambient login.** Most runtimes can
> use whatever login you already have with no setup. These three cannot,
> because a file we must own bounds their runs — Cursor's `cli-config.json`,
> the MCP `config.toml` Codex and Grok each read — and writing it into
> `~/.cursor`, `~/.codex` or `~/.grok` would edit the config your own
> terminal reads. So sign in once with
> `cma-agent runtimes:login --runtime cursor` (or `--runtime codex`,
> `--runtime grok`), even for the default profile.
...
| `workspace.desktop` | Makes `config-my-ai-workspace-<device id>` on this machine's Desktop — a fresh git repository — and shares it. The name and the path are composed here; the server sends neither |
...
| --- | --- |
| `src/runtimes/claude-code.js` | Claude Code 2.1.270 — `claude --help` plus two live `-p` runs, whose real output is the test fixtures |
| `src/runtimes/cursor.js` | dated third-party captures of real builds (2026.05 → 2026.08) and Cursor's own published SDK types; no build was run here |
| `src/runtimes/codex.js` | exec-mode experiments against codex 0.114.0 (flag surface, JSONL events), re-checked against `codex-rs/exec/src/cli.rs` on main |
| `src/runtimes/gemini.js` | flags read out of the shipped bundle of `@google/gemini-cli` 0.54.4 |
| `src/runtimes/grok.js` | the headless-mode and permissions pages of the user guide that ships with the CLI's source, the clap definitions in `src/app/cli.rs`, and the event projector that names the usage fields (`notification.rs`) |
| `src/runtimes/antigravity.js` | agy's official CHANGELOG (1.0.0 → 1.2.2), the flag table its own shell completion generates, and stream captures third-party harnesses published for builds 1.1.13 → 1.2.2; no build was run here |
| `src/runtimes/codewhale.js` | CodeWhale's published launch contract for `codewhale exec` (docs/AGENT_RUNTIME.md in its repository), cross-checked against the clap definitions at v0.9.13, including the NDJSON event vocabulary |
...
`agent/test/runtimes.test.js` asserts that every runtime draws the SAME
boundary in its own syntax — Claude Code and Grok Build as flags
(`--allowedTools` / `--allow` and `--deny`), Gemini as `--approval-mode` plus
`--allowed-tools`, Cursor as a deny list in `cli-config.json` next to `--force`
and its sandbox, Codex and CodeWhale as the sandbox mode each turn runs under.
Cursor is the one with no allow list to assert: `--force` is "force allow
commands unless explicitly denied", so under it an allow list decides nothing
and only the denies and the sandbox bound the run. That is the test that
catches "we accidentally gave Cursor a shell".
...
## Hosted mode: a machine Configure My AI runs for you

Everything above assumes your own computer. Pick **Devices → Use your own
subscription → A cloud machine we run** in the web app and the same companion
runs on a small machine we stand up instead — created with the CLI and
`cma-agent` installed, paired by itself, signed in through a relay because it
has no screen, and asleep whenever it is not working. The server side is
`app/services/cloud_companions/`; this section is what the companion does
differently there, all of it behind `CMA_HOSTED=1` (set by the machine's boot
script) and none of it reachable on a laptop.

| What | How |
| --- | --- |
| **It stops when idle.** | After `CMA_HOSTED_IDLE_MS` (10 minutes) with no job and no sign-in in flight, it asks the server whether work is on the way (`POST /api/agent/v1/hosted/sleep`), and if not it exits 0. The boot script treats that as "stop the machine". The server starts the machine again when a job is queued, and the companion that boots claims it within seconds. |
| **It signs in by relay.** | A `login.start` job runs the vendor's no-browser flow (`claude setup-token`, `codex login --device-auth`, `cursor-agent login`, `grok login --device-auth`, `agy`) under a pseudo-terminal, pulls the sign-in link — and a device code, when there is one — out of what it prints, and posts them to the server for the person to open anywhere. A `login.code` job types the code they pasted into the flow's stdin. When the flow ends the login is probed, the verdict posted, and the logins re-reported so the provider appears. |
| **Claude Code's token stays here.** | `setup-token` prints a long-lived token instead of storing one. The companion keeps it in `$CMA_AGENT_HOME/hosted-auth.json` (mode 0600, on the machine's persistent disk) and hands it to every `claude` spawn as `CLAUDE_CODE_OAUTH_TOKEN`. It is never sent anywhere. |
| **It says what the subscription is doing.** | A failed run's message is classified (`src/hosted_status.js`) — rate limited, sign in needed, out of included usage, a billing refusal, an account refusal, vendor unreachable — and rides with the failure so the web app shows a status and a button. The login scans report the same verdict per login. |
| **It updates itself.** | At boot and every 24 hours it asks `GET /api/agent/v1/hosted/updates` whether the runtime or the companion is behind; when idle it runs the install commands the server named and exits 75, which the boot script treats as "start me again". `cma-agent hosted:update` does the same by hand. |

The machine's disk holds everything that has to outlive a restart — `HOME`
(the vendor's own login), npm's prefix (the tools) and `CMA_AGENT_HOME` — so
the root filesystem can be replaced without signing anyone out.

## Limits

- A turn that cannot continue an earlier session — the first one, an edited or
  shortened history, a changed model, folder or machine — sends the whole
  conversation flattened into one prompt and pays for all of it. A turn that
  can resume continues the CLI's own transcript and sends only the new
  messages, so the history is a cache read on the vendor's side rather than a
  re-send.
- On your own computer, work only runs while it is awake and online. Chat,
  code sessions and the composer are fine; embedded widgets, API endpoints and
  scheduled bots need a credential that works without you — or a cloud
  machine (above), which wakes itself.
