...
`cma-agent runtimes:login --runtime grok`. Two things are worth knowing before
you pick it: the prompt travels as a command-line argument (visible in `ps` on
your own machine, and bounded by ARG_MAX on a very long conversation), and the
token counts come from an event whose field names xAI has not published yet.
